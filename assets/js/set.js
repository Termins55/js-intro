...
brands.forEach(
  (b) =>
    (phonesWithUsers[b] = users.filter((u) => u.brand === b).map((u) => u.name))
);
console.log(phonesWithUsers)
// отримати всі об'єкти, де brand: 'IPhone'
// const usersWithPhones = users.filter((u) => u.brand === "IPhone");
// const userNamesWithIPhone = usersWithPhones.map((u) => u.name);
